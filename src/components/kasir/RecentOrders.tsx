    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md max-h-[80vh] flex flex-col shadow-xl">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="font-heading font-bold text-lg">📋 Order Hari Ini</h3>
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center hover:bg-gray-200">✕</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            ...
            <p className="text-center text-gray-400 text-sm py-8">Belum ada order hari ini</p>
          ) : (
            orders.map((order) => (
              <div key={order.id} className={`p-3 rounded-xl border transition-all ${order.status === "cancelled" ? "border-red-200 bg-red-50 opacity-60" : "border-gray-200 hover:border-gray-300"}`}>
                ...
                <p className="text-xs text-gray-500 mb-1">{order.items_summary}</p>
